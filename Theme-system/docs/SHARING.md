...
Packs are looked for in four places, and the first one that has a given id wins:

1. `IPM_THEMES_DIR`
2. the `themes` folder next to the app (`main.py` / the `.pyz`)
3. `~/.ipm/themes` (what `install` writes to)
4. `themes/` **inside** the running `.pyz` or PyInstaller bundle - the packs that
   shipped with the download, which a personal copy always overrides.
